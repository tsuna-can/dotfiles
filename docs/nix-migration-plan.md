...
- [x] Nix 管理に移行した formula と、削除判断した formula / cask を `brew uninstall`
  （formula は全128個削除し Homebrew は cask 専用に。cask は通常 uninstall で設定を保持）
- [x] tap をすべて削除（8個）
- [x] Volta / sdkman / rbenv の残骸を削除（`~/.volta` `~/.sdkman` `~/.rbenv`）
- [x] `brew autoremove && brew cleanup`（約916MB解放）
- [x] 削除済みアプリを参照するログイン項目（Podman Desktop）を削除

注: Caskroom の `docker` は `docker-desktop` への別名シンボリックリンク（リネームの名残）で
実体は同一。削除すると Docker.app が消えるため放置してよい。
