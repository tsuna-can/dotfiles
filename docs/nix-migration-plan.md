...
5. `sudo nix run nix-darwin -- switch --flake .#default`
6. `make link`（シンボリックリンク作成 — 既存の link.sh 群はそのまま使える）
7. `make mac-config`（macOS defaults 設定）
8. claude-code を公式インストーラーで導入: `curl -fsSL https://claude.ai/install.sh | bash`
9. Step 6 のチェックリストで動作確認
